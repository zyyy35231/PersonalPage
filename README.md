...
In the workbench, click a photo to preview it and move the active frame. Press Space on the active photo to toggle selected/unreviewed, use arrow keys to move through the grid, and use 1/2/3/0 for selected/maybe/rejected/unreviewed. Changes auto-save, and the Save button writes the current photo state, series, and note immediately.
